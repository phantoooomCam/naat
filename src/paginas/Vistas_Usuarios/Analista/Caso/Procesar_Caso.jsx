...
const ProcesamientoView = ({ isSidebarCollapsed }) => {
  ...
  const handleCrearCaso = async () => {
    ...
    // Validar selects según nivel - Comentar temporalmente estas validaciones para pruebas
    /*
    ...
    */
    ...
    try {
      ...
      const casoData = {
        ...
        idUsuario: idUsuario || 1, // Proporcionar un ID por defecto si no hay usuario
      }

      // Añadir campos según nivel
      if (userLevel === 1) {
        casoData.idOrganizacion = selectedOrg || "1"
        casoData.idDepartamento = selectedDept || "1"
        casoData.idArea = selectedArea || "1"
      } else if (userLevel === 2) {
        casoData.idDepartamento = selectedDept || "1"
        casoData.idArea = selectedArea || "1"
      } else if (userLevel === 3) {
