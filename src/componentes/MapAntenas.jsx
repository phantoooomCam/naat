...
const MapAntenas = ({ idSabana, apiBase = "", height = "100%", filtros = {} }) => {
  ...
  useEffect(() => {
    ...
    const fetchAntenas = async () => {
      try {
        ...
        const url = `${apiBase}/api/sabanas/${idSabana}/registros/coordenadas-decimales${queryString ? `?${queryString}` : ""}`
  ...
  useEffect(() => {
    ...
    const unique = new Set(antenas.map((a) => (a.rank == null ? "__SIN_RANK__" : String(a.rank))))
  ...
  const filteredAntenas = useMemo(() => {
    ...
    return antenas.filter((a) => selectedRanks.has(a.rank == null ? "__SIN_RANK__" : String(a.rank)))
  ...
  const fitBounds = useCallback(() => {
    ...
    if (filteredAntenas.length === 1) {
      ...
      filteredAntenas.forEach((a) => bounds.extend({ lat: a.latitudDecimal, lng: a.longitudDecimal }))
  ...
  const uniqueRanksSorted = useMemo(() => {
    const set = new Set(antenas.map((a) => (a.rank == null ? "__SIN_RANK__" : String(a.rank))))
            ...
            {filterOpen && (
              <div className="mapa-antenas-filter-panel" role="dialog" aria-label="Filtro de ranks">
                <div className="filter-header">
                  ...
                  <button type="button" className="close-btn" onClick={() => setFilterOpen(false)} aria-label="Cerrar">
                    ×
                  </button>
                </div>
                ...
                <div className="filter-list">
                  {filteredRankList.length === 0 && <div className="filter-empty">Sin coincidencias</div>}
                  {filteredRankList.map((rk) => {
                    const checked = selectedRanks.has(rk)
                    const label = rk === "__SIN_RANK__" ? "Sin rank" : rk
                    return (
                      <label key={rk} className="filter-item">
                        <input type="checkbox" checked={checked} onChange={() => toggleRank(rk)} />
                        <span>{label}</span>
                      </label>
                    )
                  })}
        ...
        {selectedAntenna && (
          ...
          >
            <div className="antenna-info">
              <h5>Antena {selectedAntenna.rank ? `#${selectedAntenna.rank}` : `#${selectedAntenna.index + 1}`}</h5>
              <p>
              ...
              {selectedAntenna.primerUso && (
                <p>
                  <strong>Primer uso:</strong> {new Date(selectedAntenna.primerUso).toLocaleString("es-MX")}
                </p>
              )}
              {selectedAntenna.ultimoUso && (
                <p>
                  <strong>Último uso:</strong> {new Date(selectedAntenna.ultimoUso).toLocaleString("es-MX")}
                </p>
              )}
              {selectedAntenna.serie && selectedAntenna.serie.length > 0 && (
                <div>
                  <strong>Actividad por hora:</strong>
                  <div style={{ maxHeight: "100px", overflowY: "auto", fontSize: "0.85em", marginTop: "4px" }}>
